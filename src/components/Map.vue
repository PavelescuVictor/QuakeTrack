        ...
        <div class="map__content">
            ...
            <div class="form__wrapper">
                <form @submit.prevent="generateQuakesData">
                    <div
                        class="form__error"
                        v-bind:class="{ notDisplayed: formErrorNull }"
                    >
...
export default {
    ...
    data() {
        return {
            ...
            mapbox: null,
            accessToken:
                "pk.eyJ1IjoiMjA0M253IiwiYSI6ImNrZG9qeWhtMDBoMGYycXFvNm9nZnh4YzMifQ.djEvGtmsohtkGeFhV-0OFg",
    ...
    methods: {
        generateQuakesData() {
            ...
            if (daysSetFlag === false)
                this.formError = "The day counter must be set!";
            else if (maxDepthSetFlag === false)
                this.formError = "The max depth counter must be set!";
            else if (minDepthSetFlag === false)
                this.formError = "The min depth counter must be set!";
            else if (minMagnitudeSetFlag === false)
                this.formError = "The min magnitude counter must be set";
            else {
                var currentdate = new Date();
                var queryDate = new Date();
                queryDate.setDate(currentdate.getDate() - this.days);
                var dateTime = `${currentdate.getFullYear()}-${currentdate.getMonth() +
                    1}-${currentdate.getDate()}T${currentdate.getHours()}:${currentdate.getMinutes()}:${currentdate.getSeconds()}`;
                var queryDateTime = `${queryDate.getFullYear()}-${queryDate.getMonth() +
                    1}-${queryDate.getDate()}T${queryDate.getHours()}:${queryDate.getMinutes()}:${queryDate.getSeconds()}`;
                axios
                    .get(
                        `https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=${queryDateTime}&endtime=${dateTime}&minmagnitude=${this.minMagnitude}&maxdepth=${this.maxDepth}&mindepth=${this.minDepth}`
                    )
                    .then((response) => {
...
.days input:hover,
.magnitude-min input:hover,
.depth-min input:hover,
.depth-max input:hover {
...
.submit input {
    ...
    transition: padding 0.2s ease-in, background 0.2s ease-in,
        color 0.2s ease-in, border-radius 0.2s ease-in;
}
